        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
                ...
                {messagesDummyData.map((msg,idx)=>(
                    <div className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse '}` } key={idx}>
                        {msg.image ? (
                            ...
                            <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all text-black ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none bg-green-500/30' : 'rounded-bl-none bg-cyan-500/30'}`}>{msg.text}
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 '>
                <div className='flex-1 flex items-center bg-gray-300 px-3 rounded-full '>
                    <input type="text" name="" value={text} onChange={(e)=>{settext(e.target.value)}} placeholder='send a message' id='' className='flex-1 p-3 rounded-lg text-black placeholder-gray-400 text-sm border-none outline-none'/>
                ...
                <img onClick={(e)=>{settext("")}} src={assets.send} alt="" className={`w-7 mr-2 cursor-pointer active:scale-85`}/>
            </div>
        </div>
    ) : (
